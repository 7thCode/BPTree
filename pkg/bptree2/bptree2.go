...
import (
	...
	"bptree2/pkg/bptree2/bnode"
	"bptree2/pkg/bptree2/bpager"
)
...
func (t *BPTree) insertLeaf(data []byte, key, value uint64) (uint64, bpager.PageID, error) {
	leaf := bnode.NewLeafNode(data, false)
...
func (t *BPTree) insertInternal(data []byte, pageID bpager.PageID, key, value uint64) (uint64, bpager.PageID, error) {
	internal := bnode.NewInternalNode(data, false)
	childID := internal.GetChildForKey(key)
