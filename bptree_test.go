...
func TestBasicOperations(t *testing.T) {
	...
	// Insert
	if err := tree.Put(10, 100); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if err := tree.Put(5, 50); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	if err := tree.Put(15, 150); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	...
	// Update
	if err := tree.Put(10, 200); err != nil {
		t.Fatalf("Put failed: %v", err)
	}
	val, _ = tree.Get(10)
	if val != 200 {
...
func TestLargeInsert(t *testing.T) {
	...
	// Insert n keys
	for i := 0; i < n; i++ {
		if err := tree.Put(uint64(i), uint64(i*10)); err != nil {
...
func TestRandomInsert(t *testing.T) {
	...
	// Insert
	for i, k := range keys {
		if err := tree.Put(k, uint64(i)); err != nil {
			t.Fatalf("Put failed: %v", err)
		}
	}
...
func TestRangeScan(t *testing.T) {
	...
	// Scan range [30, 50]
	var results []uint64
	err = tree.Scan(30, 50, func(key, value uint64) bool {
...
func TestScanEarlyStop(t *testing.T) {
	...
	count := 0
	tree.Scan(1, 100, func(key, value uint64) bool {
		count++
...
func TestPersistence(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		tree1.Put(uint64(i), uint64(i*10))
	}
	tree1.Checkpoint()
	tree1.Close()
...
func TestDelete(t *testing.T) {
	...
	val, found := tree.Get(4)
	if !found || val != 40 {
		t.Error("key 4 should still exist")
	}
	val, found = tree.Get(6)
	if !found || val != 60 {
...
func TestEmptyTree(t *testing.T) {
	...
	// Scan empty tree
	count := 0
	tree.Scan(0, 100, func(key, value uint64) bool {
		count++
		return true
	})
	if count != 0 {
		t.Error("scan of empty tree should return 0 results")
	}
...
func BenchmarkScan(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		tree.Scan(1000, 2000, func(key, value uint64) bool {
			return true
		})
	}
}
