...
import (
	...
	"github.com/oda/bptree/internal/node"
...
func (t *BPTree) Put(key, value uint64) error {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	if rootID == 0 {
		...
		leaf := node.NewLeafNode(data, true)
		leaf.Put(key, value)
	...
	if newChildID != 0 {
		...
		newRoot := node.NewInternalNode(data, true)
...
func (t *BPTree) insert(pageID pager.PageID, key, value uint64) (uint64, pager.PageID, error) {
	data := t.pager.GetPage(pageID)
	if data == nil {
	...
	if nodeType == node.NodeTypeLeaf {
		return t.insertLeaf(data, key, value)
	}

	return t.insertInternal(data, pageID, key, value)
}

// insertLeaf inserts into a leaf node.
func (t *BPTree) insertLeaf(data []byte, key, value uint64) (uint64, pager.PageID, error) {
	leaf := node.NewLeafNode(data, false)
...
func (t *BPTree) insertInternal(data []byte, pageID pager.PageID, key, value uint64) (uint64, pager.PageID, error) {
	internal := node.NewInternalNode(data, false)
	childID := internal.GetChildForKey(key)
	...
	internal = node.NewInternalNode(data, false)
	newInternal := node.NewInternalNode(newData, false)
...
func (t *BPTree) deleteRecursive(pageID pager.PageID, key uint64) (bool, bool) {
	data := t.pager.GetPage(pageID)
	if data == nil {
		return false, false
	}
...
func (t *BPTree) handleUnderflow(parent *node.InternalNode, childIdx int, parentData []byte) {
	childID := parent.GetChild(childIdx)
	childData := t.pager.GetPage(childID)
	childType := node.GetNodeType(childData)
	...
	if childIdx > 0 {
		...
		if childType == node.NodeTypeLeaf {
			leftSib := node.NewLeafNode(leftSibData, false)
			if leftSib.CanLendTo() {
				child := node.NewLeafNode(childData, false)
				newSeparator := child.BorrowFromLeft(leftSib)
				parent.SetKeyAt(childIdx-1, newSeparator)
				return
			}
		} else {
			leftSib := node.NewInternalNode(leftSibData, false)
			if leftSib.CanLendTo() {
				child := node.NewInternalNode(childData, false)
	...
	if childIdx < parent.KeyCount() {
		...
		if childType == node.NodeTypeLeaf {
			rightSib := node.NewLeafNode(rightSibData, false)
			...
			rightSib := node.NewInternalNode(rightSibData, false)
			if rightSib.CanLendTo() {
				child := node.NewInternalNode(childData, false)
...
// Scan iterates over all key-value pairs where start <= key <= end.
// The callback function is called for each pair. Return false to stop iteration.
func (t *BPTree) Scan(start, end uint64, fn func(key, value uint64) bool) error {
	t.mu.RLock()
	defer t.mu.RUnlock()
...
func (t *BPTree) findLeaf(pageID pager.PageID, key uint64) pager.PageID {
	data := t.pager.GetPage(pageID)
	if data == nil {
		return 0
	}
